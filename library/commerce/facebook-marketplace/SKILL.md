...
description: "A read-mostly Marketplace CLI with local watches, seller workflow helpers, and explicit write checkpoints. Trigger phrases: `search Facebook Marketplace`, `watch Marketplace listings`, `draft a Marketplace listing`, `reply to a Marketplace buyer`."
author: "user"
license: "Apache-2.0"
...
- **`watch add`** — Persist a Marketplace search watch with deterministic keyword, price, and distance filters.
  ...
  ```
- **`matches`** — Show new watch matches after deterministic filtering and one LLM relevance pass on survivors.
...
- `facebook-marketplace-pp-cli matches` — Show watcher matches after deterministic filtering and one LLM relevance pass.
